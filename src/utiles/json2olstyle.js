...
const convertirNode = (key, node) => {
  ...
  let conversion = {}
  conversion[equivalencias[key]['key']] = new equivalencias[key]['class'](
    newnode
  )
  return conversion
}

const serializedStyleIfHighlight = function (
  originalSerializedStyle,
  newStyle
) {
...
const prepareSimplePointVectors = function (targetStyle, sourceStyle) {
  ...
  if ('circle' in targetStyle.style && '_simple_point' in sourceStyle.style) {
    targetStyle.style['circle'] = {
      ...targetStyle.style['circle'],
      ...sourceStyle.style['_simple_point'],
    }
    return
  }
  if ('square' in targetStyle.style && '_simple_point' in sourceStyle.style) {
    targetStyle.style['square'] = {
      ...targetStyle.style['square'],
      ...sourceStyle.style['_simple_point'],
    }
    return
...
const joinDefaultValuesWithNewValuesInPoints = function (
  targetStyle,
  sourceStyle,
  keyShape
) {
  if (keyShape in targetStyle.style) {
    targetStyle.style[keyShape] = {
      ...targetStyle.style[keyShape],
      ...sourceStyle.style[keyShape],
    }
 ...
 * @returns
 */
const serializedStyleSetTexture = function (
  serializedStyle,
  posibleShapePoint,
  textureOptions
) {
  ...
  if (
    serializedStyle.style[posibleShapePoint] &&
    serializedStyle.style[posibleShapePoint].fill
  ) {
...
const removeFillStyle = function (
  serializedStyle,
  remove = 'fill',
  posibleShapePoint = 'circle'
) {
  ...
  if (
    serializedStyle.style[posibleShapePoint] &&
    serializedStyle.style[posibleShapePoint][remove]
  ) {
